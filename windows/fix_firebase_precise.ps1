# Precise Firebase CMake Fix for Windows
Write-Host "====================================================="
Write-Host "Precise Firebase CMake Fix for Windows"
Write-Host "====================================================="

$plugin_dir = "flutter\ephemeral\.plugin_symlinks"

# Fix plugin CMakeLists.txt files
$plugins = @(
    @{
        name = "firebase_core";
...
foreach ($plugin in $plugins) {
    ...
    if (Test-Path $plugin_dir_path) {
        ...
        if (Test-Path $cmake_path) {
            Copy-Item $cmake_path "$cmake_path.precise_backup" -Force
        }
...
if (Test-Path $firebase_core_cpp) {
    ...
    # Create backup
    Copy-Item $firebase_core_cpp "$firebase_core_cpp.precise_backup" -Force
...
Write-Host "====================================================="
Write-Host "All Firebase CMake files have been precisely fixed"
