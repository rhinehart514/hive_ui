...
class HiveNavigationDemo extends StatefulWidget {
  const HiveNavigationDemo({super.key});

  @override
  State<HiveNavigationDemo> createState() => _HiveNavigationDemoState();
}

class _HiveNavigationDemoState extends State<HiveNavigationDemo> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'HIVE Navigation Demo',
      theme: ThemeData.dark(),
      home: HiveAdaptiveNavigation(
          ...
          child: Center(
            child: Column(
              ...
              children: [
                ...
                if (_navigationItems[_selectedIndex].hasLiveContent)
                  Container(
...
// Live Indicator Widgets
class LiveIndicatorRing extends StatefulWidget {
  final Widget child;
  final bool isActive;

  const LiveIndicatorRing({
    super.key,
    required this.child,
    this.isActive = false,
  });
  ...
  @override
  State<LiveIndicatorRing> createState() => _LiveIndicatorRingState();
}

class _LiveIndicatorRingState extends State<LiveIndicatorRing>
  ...
  @override
  void didUpdateWidget(LiveIndicatorRing oldWidget) {
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                border: Border.all(
                  color: const Color(0xFFFFD700).withOpacity(_animation.value),
                  width: 2,
                ),
              ),
            ),
...
class ProgressStemIndicator extends StatelessWidget {
  final Widget child;
  final double progress;
  final bool isActive;

  const ProgressStemIndicator({
    super.key,
    required this.child,
    this.progress = 0.5,
    this.isActive = false,
...
// Ambient Glow Effect
class AmbientGlowEffect extends StatefulWidget {
  final Widget child;
  final bool isActive;
  final Color glowColor;

  const AmbientGlowEffect({
    super.key,
    required this.child,
    this.isActive = false,
  ...
  @override
  State<AmbientGlowEffect> createState() => _AmbientGlowEffectState();
}

class _AmbientGlowEffectState extends State<AmbientGlowEffect>
    with SingleTickerProviderStateMixin {
  late AnimationController _controller;
  ...
  @override
  void didUpdateWidget(AmbientGlowEffect oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.isActive && !oldWidget.isActive) {
      _controller.repeat(reverse: true);
    } else if (!widget.isActive && oldWidget.isActive) {
