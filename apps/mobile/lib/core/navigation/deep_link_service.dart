...
class DeepLinkService {
  ...
  Future<void> _handleDeepLink(String link) async {
    ...
    try {
      ...
      // --- Check for Email Verification Action Link FIRST ---
      final mode = uri.queryParameters['mode'];
      final oobCode = uri.queryParameters['oobCode'];

      if (mode == 'verifyEmail' && oobCode != null && oobCode.isNotEmpty) {
        debugPrint('DeepLinkService: Detected email verification link.');
        try {
          final authRepo = _ref.read(authRepositoryProvider);
          await authRepo.applyActionCode(oobCode);
          debugPrint('DeepLinkService: Email verification code applied successfully.');
          // Navigate to success page
          _navigateToPath('/verified-email');
        } catch (e) {
          debugPrint('DeepLinkService: Error applying email verification code: $e');
          // Navigate to error page
          _navigateToPath('/verification-error');
        }
        _isHandlingDeepLink = false; // Done processing this specific link type
        return; // Stop further processing
      }
      // --- End Email Verification Check ---

      // Check the scheme (for non-verification links)
      if (uri.scheme != kCustomScheme && uri.host != 'hiveapp.com') {
        debugPrint('DeepLinkService: Invalid scheme or host for non-verification link: ${uri.scheme}');
  ...
  /// Helper to navigate using GoRouter
  void _navigateToPath(String path, {Object? extra}) {
     try {
       final navKey = _ref.read(navigatorKeyProvider); // Use existing navigatorKeyProvider
       final context = navKey.currentContext;
       if (context != null && GoRouter.of(context).routeInformationProvider.value.uri.toString() != path) {
         debugPrint('DeepLinkService: Navigating to path: $path');
         GoRouter.of(context).go(path, extra: extra);
       } else {
         debugPrint('DeepLinkService: Navigation skipped - context is null or already at path $path');
       }
     } catch (e) {
       debugPrint('DeepLinkService: Error during navigation to $path: $e');
     }
  }
}
