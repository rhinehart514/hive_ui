...
final realtimeMessagingServiceProvider = Provider<RealtimeMessagingService>((ref) {
  final service = RealtimeMessagingService();
  
  // Initialize the service when it's created, but only if needed
  WidgetsBinding.instance.addPostFrameCallback((_) async {
    try {
      // Always try to initialize - the service will handle if it's already initialized
      await service.initialize();
      debugPrint('✅ RealtimeMessagingService initialization triggered by provider');
    } catch (e) {
      debugPrint('⚠️ Error during RealtimeMessagingService initialization: $e');
    }
  });
