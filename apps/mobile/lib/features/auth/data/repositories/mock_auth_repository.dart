...
class MockAuthRepository implements AuthRepository {
  ...
  Future<void> linkEmailPassword(String email, String password) async {
    ...
    if (userEntry.key.isNotEmpty) {
      ...
      // Create updated user (result is potentially nullable)
      final AuthUser updatedUserOrNull = _currentUser.copyWith(email: normalizedEmail);
      
      // Ensure non-null before assigning, throw if unexpectedly null
      final updatedUser = updatedUserOrNull ?? 
          (throw AuthException('Failed to update user during email link: copyWith returned null unexpectedly.'));
      
      // Add with new email and password
      _users[normalizedEmail] = _MockUserCredential(updatedUser, password);
      
      // Update current user - now assigning a guaranteed non-null AuthUser
      _currentUser = updatedUser; 
      _authStateController.add(_currentUser);
  ...
  Future<void> applyActionCode(String code) async {
    // Mock implementation: Succeed for a specific code, fail otherwise
    if (code == 'valid_code' || code == 'verify_email_code') {
      // Simulate success
      _currentUser = _currentUser.copyWith(isEmailVerified: true);
      _authStateController.add(_currentUser);
      return;
    } else {
      // Simulate failure (e.g., invalid code)
  ...
  Future<bool> sendSignInLinkToEmail(String email) async {
    print('Mock: Sending sign-in link to $email');
    return true;
  }

  @override
  Future<bool> isSignInWithEmailLink(String link) async {
    print('Mock: Checking if link is valid: $link');
    // Basic mock logic: succeed if simulateNetworkDelay is true and link is not empty
    return simulateNetworkDelay && link.isNotEmpty;
  }

  @override
  Future<User?> signInWithEmailLink(String email, String link) async {
    print('Mock: Signing in with email link for $email');
    if (simulateNetworkDelay && email.isNotEmpty && link.isNotEmpty) {
      _currentUser = AuthUser(
        id: 'magic_link_user_id',
        email: email,
        displayName: 'Magic Link User',
        isEmailVerified: true, // Assume verified by link
        createdAt: DateTime.now(),
        lastSignInTime: DateTime.now(),
      );
      // Return a mock Firebase User - requires firebase_auth dependency for User type
      // For simplicity, returning null here but ideally return a mocked User
      return null; 
    } else {
      throw Exception('Mock magic link sign-in failed');
    }
  }
  ...
  Future<AuthUser> registerWithPasskey(String email) {
    // TODO: implement registerWithPasskey
    throw UnimplementedError();
  }
  
  @override
  Future<AuthUser> signInWithPasskey() {
    // TODO: implement signInWithPasskey
    throw UnimplementedError();
  }
}
