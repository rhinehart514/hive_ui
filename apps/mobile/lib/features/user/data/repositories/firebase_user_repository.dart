...
class FirebaseUserRepository implements UserRepository {
  ...
  @override
  Future<void> updateUserProfile(String userId, UserProfile profileData) async {
    try {
      // Prepare data using toJson, ensuring it only contains fields to update.
      // The UserProfile constructor should handle setting default/null values correctly.
      final dataToUpdate = profileData.toJson();
      
      // Remove fields that shouldn't be directly set during onboarding update
      dataToUpdate.remove('id'); // ID is the document key
      dataToUpdate.remove('createdAt'); // Should likely be set on creation
      // Add updatedAt timestamp
      dataToUpdate['updatedAt'] = FieldValue.serverTimestamp();
      // Add displayNameLowercase if not already present
      if (dataToUpdate.containsKey('displayName') && dataToUpdate['displayName'] != null) {
        dataToUpdate['displayNameLowercase'] = 
            (dataToUpdate['displayName'] as String).toLowerCase();
      }

      // Use set with merge:true to update only the provided fields
      await _usersCollection.doc(userId).set(dataToUpdate, SetOptions(merge: true));
      
      // TODO: Replace debug print with FirebaseMonitor write record if available
      // FirebaseMonitor.recordWrite(); 
