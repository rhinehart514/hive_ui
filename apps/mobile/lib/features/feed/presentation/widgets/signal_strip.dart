...
class _SignalStripState extends ConsumerState<SignalStrip> 
  ...
  void _ensureInitialized() {
    // Check and ensure core app initialization is complete
    ref.read(appInitializationProvider.future).then((_) {
       // Future completed without error, implies successful initialization
       // This debug print might be verbose, consider removing if not needed
       debugPrint('SignalStrip: Core app initialization verified successfully.');
    }).catchError((error, stackTrace) {
       // The FutureProvider's error handling might already cover this.
       // Adding specific context here can be helpful for debugging SignalStrip issues.
       debugPrint('Warning: Core app initialization failed before SignalStrip loaded. Error: $error');
       // Optionally, update widget state here to show an error/disabled state for the strip
    });
  }
