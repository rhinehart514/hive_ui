...
class _SpacesPageState extends ConsumerState<SpacesPage> with TickerProviderStateMixin {
  ...
  void _scrollToNewSpace(String spaceId, List<entities.SpaceEntity> spaces) {
    ...
    const double itemWidth = (_mySpaceCircleRadius * 2) + (_mySpaceHorizontalPadding * 2);
  ...
  Widget _buildMySpacesSection() {
    ...
    return userSpacesAsync.when(
      data: (userSpacesEntities) {
        ...
        const double circleDiameter = _mySpaceCircleRadius * 2; // 72.0
        const double spacingBelowCircle = 6.0;
        const double labelHeightEstimate = 30.0; // Approx height for max 2 lines of text
        const double itemVerticalPadding = 8.0; // Extra buffer within the list item height
        const double listHeight = circleDiameter + spacingBelowCircle + labelHeightEstimate + itemVerticalPadding; // Height for the SizedBox containing the ListView

        // Height needed for the separate "You're not in any Spaces..." text widget
        const double emptyStateTextTopPadding = 12.0;
        const double emptyStateTextHeightEstimate = 20.0; 
        final double emptyStateWidgetHeight = isEmptyState ? (emptyStateTextTopPadding + emptyStateTextHeightEstimate) : 0.0;

        // Total container height
        const double containerVerticalPadding = 16.0; // Top/Bottom padding for the outer Container
        ...
        return Container(
          ...
          padding: const EdgeInsets.symmetric(vertical: containerVerticalPadding), // Use correct padding var
          child: Column(
            ...
            children: [
              ...
              if (isEmptyState)
                Padding(
                  ...
                  padding: const EdgeInsets.only(top: emptyStateTextTopPadding),
      ...
      loading: () {
        ...
        const double loadingListHeight = (_mySpaceCircleRadius * 2) + 6.0 + 30.0 + 8.0;
        const double loadingContainerHeight = loadingListHeight + (16.0 * 2);
        return Container(
      ...
      error: (error, stackTrace) {
        ...
        const double errorListHeight = (_mySpaceCircleRadius * 2) + 6.0 + 30.0 + 8.0;
        const double errorContainerHeight = errorListHeight + (16.0 * 2);
        return Container(
...
class _DiscoverSpaceCardState extends ConsumerState<DiscoverSpaceCard> with TickerProviderStateMixin { // Use TickerProviderStateMixin for multiple controllers
  ...
  Widget _buildJoinButton(bool isJoined) {
    ...
    const Color buttonBackgroundColor = AppColors.dark3; // Or determine based on card bg
